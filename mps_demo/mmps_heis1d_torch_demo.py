...
Nsites = 80
fpath = f'datasets/mps_cos_heis_data_L{Nsites}_D3.h5'
with h5py.File(fpath, 'r') as f:
    alldata = torch.tensor(f['heis-bd-data'][:-4,:,:])
...
num_instances = alldata.shape[0]
# truncate the data to get a different model
truncation_factors = np.geomspace(0.01, 1, 10)[:-1]
truncation_limits = np.round(truncation_factors * num_instances).astype('int')

seed = 78910
rng = np.random.default_rng(seed)
dataset_indices = [rng.permutation(num_instances)[:x] for x in truncation_limits]

for j_trunc in range(len(truncation_limits)):
    
    nsamples = truncation_limits[j_trunc]
    data = alldata[dataset_indices[j_trunc],:,:]
    ...
    # model = NeuralNetwork()
    # model.load_state_dict(torch.load('models/L80model.pt'))
    # model.eval()
    torch.save(model, f'models/L80model_samples{nsamples}.pt')
