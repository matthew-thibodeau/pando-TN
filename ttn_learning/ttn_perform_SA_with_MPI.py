...
This script is designed to be launched on multiple MPI processes.
The runs are divided equally among the MPI tasks.
...
####################################################################
# MPI variables and utility functions
####################################################################
...
def mpi_print(*args, who: str = 'main', flush: bool = True, end: str = '\n'):
    if who=='main':
        if (mpi_rank == 0):
            print(*args, flush=flush, end=end)
    elif who=='all':
        print('[rank {:2g}]'.format(mpi_rank), *args, flush=flush, end=end)
    else:
        print(f'ERR: unknown argument who={who} when printing with mpi')
