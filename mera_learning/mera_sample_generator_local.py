...
def update_mera_local(mera, localtn, unitags):
    ...
    for iso in isoneighbors:
        ...
        newiso.unitize(inplace=True, method='exp')
...
for run in range(runs):  # 'runs' --> 'range(runs)' npds 2023-08-21
    ...
    this_id =  (f'MERA_{jobid}_{run}_L{L}_D{site_dim}')
    ...
    for i in range(0, L):
        if HAMTYPE == 'ising_random_fields':
        ...
        elif HAMTYPE == 'heis_random_bonds':
        ...
        elif HAMTYPE == 'heis_random_fields':
            ...
            terms[(i, (i+1)%L)] =  H2 + coupling_vals[i] * Z1I2
            
            
        elif HAMTYPE == 'heis_nn_random_bonds':
    ...
    for j in range(numtruncs):
        # truncate the smallest singular values
        q = get_entanglement_spectra(emo)
